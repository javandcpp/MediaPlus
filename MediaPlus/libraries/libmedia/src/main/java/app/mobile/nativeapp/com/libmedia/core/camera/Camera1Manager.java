...
import app.mobile.nativeapp.com.libmedia.core.utils.DeviceUtils;
    ...
    public void openCamera(final Integer cameraId,
        ...
        try {
            ...
            if (cameraOpenListener != null) {
                cameraOpenListener.onCameraOpened(cameraId, mPreviewSize, sufaceHolderCallback);
            }
        } catch (Exception error) {
            LogUtils.d(TAG, "Can't open camera: " + error.getMessage());
            if (cameraOpenListener != null) {
...
//        });
    ...
    protected void prepareCameraOutputs() {
        try {
            ...
            Point bestPreviewResolution = findBestPreviewResolution(camera.getParameters(), new Point(DeviceUtils.getScreenWidth(mContext), DeviceUtils.getScreenHeight(mContext)), cameraConfigProvider.getActivity().getWindowManager().getDefaultDisplay().getRotation(), 90);
//            if (bestPreviewResolution.x > mVideoSize.getWidth() || bestPreviewResolution.y > mVideoSize.getHeight()) {
//                mVideoSize.setWidth(bestPreviewResolution.x);
//                mVideoSize.setHeight(bestPreviewResolution.y);
//            }
...
//            } else {
            mPreviewSize = CameraUtils.getSizeWithClosestRatio(previewSizes, bestPreviewResolution.x, bestPreviewResolution.y);
//            }
        } catch (Exception e) {
...
//                parameters.setRecordingHint(true);
            ...
            LogUtils.d(TAG, "Error setting camera preview: " + error.getMessage());
        } catch (Exception ignore) {
            LogUtils.d(TAG, "Error starting camera preview: " + ignore.getMessage());
        }
    }
    ...
    protected void onPictureTaken(final byte[] bytes, Camera camera, final OnCameraResultListener callback) {
        ...
        if (pictureFile == null) {
            LogUtils.d(TAG, "Error creating media file, check storage permissions.");
            return;
        }
        ...
        try {
            ...
            LogUtils.e(TAG, "File not found: " + error.getMessage());
        } catch (IOException error) {
            LogUtils.e(TAG, "Error accessing file: " + error.getMessage());
        } catch (Throwable error) {
            LogUtils.e(TAG, "Error saving file: " + error.getMessage());
        }
        ...
        try {
            ...
            LogUtils.e(TAG, "Can't save exif info: " + error.getMessage());
        }
    }
